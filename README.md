...
This API provides access to the 5th Edition Dungeons and Dragons System Reference Document (SRD).

Learn more about Dungeons and Dragons API at [dnd5eapi.co](https://dnd5eapi.co).
...
This SDK exposes the API as a small set of **semantic entities** — Class, Feature, Monster and Spell — that you
...
const items = await client.Class().list()
```
...
```go
client := sdk.Test()
result, err := client.Class(nil).List(
    nil, nil,
)
```
...
| --- | --- | --- |
| **Class** | The Class entity (list, load). | `/classes` |
| **Feature** | The Feature entity (list, load). | `/features` |
| **Monster** | The Monster entity (list, load). | `/monsters` |
| **Spell** | The Spell entity (list, load). | `/spells` |

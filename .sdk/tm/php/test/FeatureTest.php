...
// DungeonsAndDragonsTwo SDK feature test
//
...
class FtCtrl extends DungeonsAndDragonsTwoControl
{
...
class FtClient
{
    ...
    private DungeonsAndDragonsTwoUtility $utility;

    public function __construct(DungeonsAndDragonsTwoUtility $utility, array $options)
    {
    ...
    public function get_utility(): DungeonsAndDragonsTwoUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (DungeonsAndDragonsTwoContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public DungeonsAndDragonsTwoContext $rootctx;
    ...
    public function __construct(
        ...
        $utility = new DungeonsAndDragonsTwoUtility();
        $this->utility = $utility;

        $utility->fetcher = $server ?? function (DungeonsAndDragonsTwoContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = DungeonsAndDragonsTwoConfig::make_feature($name);
    ...
    private static function buildUrl(DungeonsAndDragonsTwoSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof DungeonsAndDragonsTwoError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new DungeonsAndDragonsTwoResponse($fetched);
        }
    ...
    private function fail(DungeonsAndDragonsTwoContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (DungeonsAndDragonsTwoContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;

...
[MCP](https://modelcontextprotocol.io) server exposing the DungeonsAndDragonsTwo SDK as
two agent tools — `dungeons-and-dragons-two_list` and `dungeons-and-dragons-two_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/dungeons-and-dragons-two-mcp)
make build
...
```jsonc
// dungeons-and-dragons-two_list: first page of records
{ "entity": "class" }
{ "entity": "class", "query": { } }

// dungeons-and-dragons-two_load: one record by id
{ "entity": "class", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/dungeons-and-dragons-two-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user dungeons-and-dragons-two \
     -- "$PWD"/dist/*/dungeons-and-dragons-two-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `dungeons-and-dragons-two_list` and `dungeons-and-dragons-two_load` tools now appear
   in new sessions. Ask the agent to *"list class using dungeons-and-dragons-two"*
   and it calls `dungeons-and-dragons-two_list` with `{"entity":"class"}`.
...
### Call the `dungeons-and-dragons-two_list` tool
...
### Call the `dungeons-and-dragons-two_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `dungeons-and-dragons-two_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `dungeons-and-dragons-two_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |

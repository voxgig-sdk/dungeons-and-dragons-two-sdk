...
require("dungeons-and-dragons-two_types")
...
local DungeonsAndDragonsTwoSDK = {}
DungeonsAndDragonsTwoSDK.__index = DungeonsAndDragonsTwoSDK
...
DungeonsAndDragonsTwoSDK._make_feature = _make_feature


function DungeonsAndDragonsTwoSDK.new(options)
  local self = setmetatable({}, DungeonsAndDragonsTwoSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function DungeonsAndDragonsTwoSDK:options_map()
...
function DungeonsAndDragonsTwoSDK:get_utility()
  return Utility.copy(self._utility)
end


function DungeonsAndDragonsTwoSDK:get_root_ctx()
  return self._rootctx
end


function DungeonsAndDragonsTwoSDK:prepare(fetchargs)
...
function DungeonsAndDragonsTwoSDK:direct(fetchargs)
...
function DungeonsAndDragonsTwoSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function DungeonsAndDragonsTwoSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "DungeonsAndDragonsTwoSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function DungeonsAndDragonsTwoSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function DungeonsAndDragonsTwoSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "DungeonsAndDragonsTwoSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function DungeonsAndDragonsTwoSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = DungeonsAndDragonsTwoSDK.new(sdkopts)
  sdk.mode = "test"
...
return DungeonsAndDragonsTwoSDK

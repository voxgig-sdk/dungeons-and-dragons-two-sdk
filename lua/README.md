...
The Lua SDK for the DungeonsAndDragonsTwo API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Class(nil):list(nil, nil)
...
```lua
local result, err = client:Class(nil):load({ id = "example_id" }, nil)
if err then error(err) end
print(result)
```
...
local result, err = client:DungeonsAndDragonsTwo(nil):load(
  { id = "test01" }, nil
)

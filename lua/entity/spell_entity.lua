...
local SpellEntity = {}
SpellEntity.__index = SpellEntity


function SpellEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, SpellEntity)
  self._name = "spell"
  self._client = client
...
function SpellEntity:get_name()
  return self._name
end


function SpellEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return SpellEntity.new(self._client, opts)
end
...
function SpellEntity:mark_deleted()
  self._deleted = true
end


function SpellEntity:deleted()
  return true == self._deleted
end


function SpellEntity:data_set(args)
  if args ~= nil then
...
function SpellEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function SpellEntity:match_set(args)
  if args ~= nil then
...
function SpellEntity:match_get()
...
function SpellEntity:stream(action, args, callopts)
...
function SpellEntity:_run_op(ctx, post_done)
  local utility = self._utility

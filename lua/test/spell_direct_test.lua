...
describe("SpellDirect", function()
  it("should direct-list-spell", function()
    local setup = spell_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-spell", setup.live and "live" or "unit")
  ...
  it("should direct-load-spell", function()
    local setup = spell_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-spell", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function spell_direct_setup(mockres)

...
class DungeonsAndDragonsTwoSDK {
  ...
  async direct(fetchargs?: any) {
    if (!this._options.allow.op.includes('direct')) {
      return {
        ok: false,
        err: new Error('DungeonsAndDragonsTwoSDK: direct: operation not allowed by' +
  ...
  async graphql(query: string, variables?: any, ctrl?: any) {
    ...
    if (!options.allow.op.includes('graphql')) {
      return {
        ok: false,
        err: new Error('DungeonsAndDragonsTwoSDK: graphql: operation not allowed by' +
    ...
    if (null != errors && Array.isArray(errors) && 0 < errors.length) {
      ...
      const err: any = new Error('DungeonsAndDragonsTwoSDK: graphql: ' +
  ...
  // The argument is the entity OPTIONS object (passed to the entity
  // constructor as entopts), not initial entity data.
  Spell(entopts?: Record<string, any>) {
    const self = this
    return new SpellEntity(self, entopts)
  }
  ...
  static test(testoptsarg?: any, sdkoptsarg?: any) {
    ...
    const testsdk = new DungeonsAndDragonsTwoSDK(sdkopts)
    testsdk._mode = 'test'
  ...
  tester(testopts?: any, sdkopts?: any) {
    return DungeonsAndDragonsTwoSDK.test(testopts, sdkopts)
  }


  toJSON() {
    return { name: 'DungeonsAndDragonsTwo' }
  }

  toString() {
    return 'DungeonsAndDragonsTwo ' + this._utility.struct.jsonify(this.toJSON())
  }
...
const SDK = DungeonsAndDragonsTwoSDK
...
export {
  ...
  BaseFeature,
  DungeonsAndDragonsTwoEntityBase,

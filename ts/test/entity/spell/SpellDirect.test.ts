...
describe('SpellDirect', async () => {
  ...
  test('direct-load-spell', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-spell', setup.live)) return
  ...
  test('direct-list-spell', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-spell', setup.live)) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['DUNGEONS_AND_DRAGONS_TWO_TEST_SPELL_ENTID']

...
//     (\`new DungeonsAndDragonsTwoSDK(...)\` / \`DungeonsAndDragonsTwoSDK.test(...)\` become
//     \`DungeonsAndDragonsTwoSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'DungeonsAndDragonsTwoSDK'
...
const TEST_SEED = {"entity":{"class":{"test01":{"id":"test01"}},"feature":{"test01":{"id":"test01"}},"monster":{"test01":{"id":"test01"}},"spell":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new DungeonsAndDragonsTwoSDK(...)\` and any
// \`DungeonsAndDragonsTwoSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+DungeonsAndDragonsTwoSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/DungeonsAndDragonsTwoSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)

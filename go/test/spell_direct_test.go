...
func TestSpellDirect(t *testing.T) {
	t.Run("direct-list-spell", func(t *testing.T) {
		setup := spellDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-spell", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-spell", func(t *testing.T) {
		setup := spellDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-spell", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type spellDirectSetupResult struct {
	client *sdk.DungeonsAndDragonsTwoSDK
	calls  *[]map[string]any
	live   bool
	idmap  map[string]any
}

func spellDirectSetup(mockres any) *spellDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"DUNGEONS_AND_DRAGONS_TWO_TEST_SPELL_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["DUNGEONS_AND_DRAGONS_TWO_TEST_SPELL_ENTID"]; ok {
		...
		return &spellDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &spellDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}

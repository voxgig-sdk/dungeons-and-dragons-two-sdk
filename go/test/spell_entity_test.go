...
func TestSpellEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Spell(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Spell(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set DUNGEONS_AND_DRAGONS_TWO_TEST_SPELL_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func spellBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("DUNGEONS_AND_DRAGONS_TWO_TEST_SPELL_ENTID")
	...
	idmapResolved := core.ToMapAny(env["DUNGEONS_AND_DRAGONS_TWO_TEST_SPELL_ENTID"])

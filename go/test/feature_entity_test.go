...
func TestFeatureEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set DUNGEONS_AND_DRAGONS_TWO_TEST_FEATURE_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func featureBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("DUNGEONS_AND_DRAGONS_TWO_TEST_FEATURE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["DUNGEONS_AND_DRAGONS_TWO_TEST_FEATURE_ENTID"])

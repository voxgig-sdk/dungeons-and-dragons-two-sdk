...
func TestFeatureEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set DUNGEONSANDDRAGONSTWO_TEST_FEATURE_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func featureBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("DUNGEONSANDDRAGONSTWO_TEST_FEATURE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["DUNGEONSANDDRAGONSTWO_TEST_FEATURE_ENTID"])

...
module DungeonsAndDragonsTwoUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = DungeonsAndDragonsTwoResponse.new({})
    ...
    if fetch_err
      ...
      response = DungeonsAndDragonsTwoResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = DungeonsAndDragonsTwoResponse.new(fetched)
    else

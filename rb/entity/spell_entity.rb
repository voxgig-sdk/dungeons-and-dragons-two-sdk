...
class SpellEntity
  ...
  def make
    opts = @_entopts.dup
    SpellEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = DungeonsAndDragonsTwoHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Spell, Hash] the current Spell data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = DungeonsAndDragonsTwoHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Spell fields)
  def match_get
  ...
  # Load a single Spell.
  #
  # @param reqmatch [SpellLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Spell.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Spell, Hash] the loaded Spell; raises DungeonsAndDragonsTwoError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = DungeonsAndDragonsTwoHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqmatch [SpellListMatch, Hash, nil] match filter (any subset of
  #   Spell fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Spell>, Array] the matching Spell items; raises DungeonsAndDragonsTwoError on failure

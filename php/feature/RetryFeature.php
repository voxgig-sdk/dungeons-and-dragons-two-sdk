...
// DungeonsAndDragonsTwo SDK retry feature
...
class DungeonsAndDragonsTwoRetryFeature extends DungeonsAndDragonsTwoBaseFeature
{
    ...
    public function init(DungeonsAndDragonsTwoContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (DungeonsAndDragonsTwoContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(DungeonsAndDragonsTwoContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(DungeonsAndDragonsTwoContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;

...
// DungeonsAndDragonsTwo SDK Spell entity
...
class SpellEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new SpellEntity($this->_client, $opts);
    }

    /**
     * @param Spell|array $args Spell data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = DungeonsAndDragonsTwoHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Spell|array The current Spell data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Spell fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = DungeonsAndDragonsTwoHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Spell fields).
     */
     ...
     * Load a single Spell.
     *
     * @param SpellLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed SpellLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Spell|array The loaded Spell as an assoc-array at the
     *   SDK boundary; throws DungeonsAndDragonsTwoError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = DungeonsAndDragonsTwoHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
    ...
    /**
     * List Spell items matching the given filter.
     *
     * @param SpellListMatch|array|null $reqmatch Match filter (any subset
     *   of Spell fields) as an assoc-array; SpellListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Spell[]|array A list of Spell items as assoc-arrays at
     *   the SDK boundary; throws DungeonsAndDragonsTwoError on failure (item-5 convention).
     */

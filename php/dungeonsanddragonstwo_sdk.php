...
class DungeonsAndDragonsTwoSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = DungeonsAndDragonsTwoHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = DungeonsAndDragonsTwoHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ($utility->feature_add)($this->_rootctx, DungeonsAndDragonsTwoFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return DungeonsAndDragonsTwoUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = DungeonsAndDragonsTwoHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = DungeonsAndDragonsTwoHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = DungeonsAndDragonsTwoHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new DungeonsAndDragonsTwoError($op . "_allow",
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = DungeonsAndDragonsTwoHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new DungeonsAndDragonsTwoError("graphql_error",
                "DungeonsAndDragonsTwoSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function Spell($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_spell === null) {
                $this->_spell = new SpellEntity($this, null);
            }
            return $this->_spell;
        }
        return new SpellEntity($this, $data);
    }
    ...
    public static function test(?array $testopts = null, ?array $sdkopts = null): self
    {
        ...
        $sdk = new DungeonsAndDragonsTwoSDK($sdkopts);
        $sdk->mode = "test";
        return $sdk;
    }
}

...
class SpellEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = DungeonsAndDragonsTwoSDK::test(null, null);
        $ent = $testsdk->Spell(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->Spell(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Spell(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = spell_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list", "load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "spell." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set DUNGEONS_AND_DRAGONS_TWO_TEST_SPELL_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $spell_ref01_data = null;
        if (count($spell_ref01_data_raw) > 0) {
...
function spell_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/spell/SpellTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["spell01", "spell02", "spell03"] as $k) {
    ...
    $entid_env_raw = getenv("DUNGEONS_AND_DRAGONS_TWO_TEST_SPELL_ENTID");
    ...
    if ($env["DUNGEONS_AND_DRAGONS_TWO_TEST_LIVE"] === "TRUE") {

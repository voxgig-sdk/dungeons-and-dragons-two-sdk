    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set DUNGEONSANDDRAGONSTWO_TEST_FEATURE_ENTID JSON to run live")
        client = setup["client"]
        ...
        feature_ref01_data_dt0_loaded, err = feature_ref01_ent.load(feature_ref01_match_dt0, None)
        assert err is None
...
def _feature_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("DUNGEONSANDDRAGONSTWO_TEST_FEATURE_ENTID"))
